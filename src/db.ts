...
function getEncryptionKey(): Buffer {
  ...
  // Check process.env first (auto-generated keys are set there before .env is re-read)
  const hex = process.env.DB_ENCRYPTION_KEY || DB_ENCRYPTION_KEY;
...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    -- Archive table: permanent storage of all memories and conversations (never deleted)
    ...
    CREATE INDEX IF NOT EXISTS idx_archive_chat ON memory_archive(chat_id, archived_at DESC);
    CREATE INDEX IF NOT EXISTS idx_archive_source ON memory_archive(source_table, original_id);

    CREATE VIRTUAL TABLE IF NOT EXISTS consolidations_fts USING fts5(
      summary, insight, content='consolidations', content_rowid='id'
    );

    CREATE TRIGGER IF NOT EXISTS consolidations_ai AFTER INSERT ON consolidations BEGIN
      INSERT INTO consolidations_fts(rowid, summary, insight) VALUES (new.id, new.summary, new.insight);
    END;
...
export function backupDatabase(): string | null {
  try {
    ...
    const backupPath = path.join(backupDir, `claudeclaw-${date}.db`);
    ...
    const files = fs.readdirSync(backupDir)
      .filter(f => f.startsWith('claudeclaw-') && f.endsWith('.db'))
      .sort()
      .reverse();
...
export function checkDatabaseIntegrity(): boolean {
  try {
    ...
    logger.error({ err }, 'Database integrity check error');
    return false;
  }
}

/**
 * Search the permanent archive (memories and conversations that were pruned/deleted).
 */
export function searchArchive(chatId: string, query: string, limit = 10): Array<{
  id: number; original_id: number; source_table: string; content: string; metadata: string; archived_at: number; reason: string;
}> {
...
function runMigrations(database: Database.Database): void {
  ...
  // Memory soft-delete: add deleted_at column
  const memDeletedCols = database.prepare(`PRAGMA table_info(memories)`).all() as Array<{ name: string }>;
  if (!memDeletedCols.some(c => c.name === 'deleted_at')) {
    database.exec(`ALTER TABLE memories ADD COLUMN deleted_at INTEGER`);
    logger.info('Migration: added deleted_at to memories (soft-delete)');
  }

  // Mission Control: add next_mission_id column for chaining
  const missionColNames = database.prepare(`PRAGMA table_info(mission_tasks)`).all() as Array<{ name: string }>;
  if (!missionColNames.some(c => c.name === 'next_mission_id')) {
  ...
  if (queryEmbedding && queryEmbedding.length > 0) {
    ...
    if (candidates.length > 0) {
      ...
      if (scored.length > 0) {
        ...
        const rows = getDb()
          .prepare(`SELECT * FROM memories WHERE id IN (${placeholders}) AND superseded_by IS NULL AND deleted_at IS NULL`)
          .all(...ids) as Memory[];
  ...
  let results = getDb()
    .prepare(
      `SELECT memories.* FROM memories
       ...
       WHERE memories_fts MATCH ? AND memories.chat_id = ? AND memories.superseded_by IS NULL AND memories.deleted_at IS NULL
       ${agentId ? 'AND (memories.agent_id = ? OR memories.agent_id = \'main\')' : ''}
       ORDER BY (rank * memories.importance * memories.salience) ASC
       LIMIT ?`,
    )
    .all(...(agentId ? [ftsQuery, chatId, agentId, limit] : [ftsQuery, chatId, limit])) as Memory[];
  ...
  results = getDb()
    .prepare(
      `SELECT * FROM memories
       WHERE chat_id = ? AND superseded_by IS NULL AND deleted_at IS NULL AND (${likeConditions})
       ${agentId ? 'AND (agent_id = ? OR agent_id = \'main\')' : ''}
       ORDER BY (importance * salience) DESC, accessed_at DESC
       LIMIT ?`,
    )
    .all(...(agentId ? [chatId, ...likeParams, agentId, limit] : [chatId, ...likeParams, limit])) as Memory[];

  return results;
}

export function saveMemoryEmbedding(memoryId: number, embedding: number[]): void {
  getDb().prepare('UPDATE memories SET embedding = ? WHERE id = ?').run(JSON.stringify(embedding), memoryId);
}
...
export function getMemoriesWithEmbeddings(chatId: string): Array<{ id: number; embedding: number[]; summary: string; importance: number }> {
  const rows = getDb()
    .prepare('SELECT id, embedding, summary, importance FROM memories WHERE chat_id = ? AND embedding IS NOT NULL AND superseded_by IS NULL AND deleted_at IS NULL')
...
export function getRecentHighImportanceMemories(chatId: string, limit = 5): Memory[] {
  return getDb()
    .prepare(
      `SELECT * FROM memories WHERE chat_id = ? AND importance >= 0.4 AND salience >= 0.05
...
export function getRecentMemories(chatId: string, limit = 5): Memory[] {
  return getDb()
    .prepare(
      'SELECT * FROM memories WHERE chat_id = ? AND deleted_at IS NULL ORDER BY accessed_at DESC LIMIT ?',
    )
    .all(chatId, limit) as Memory[];
}
...
export function decayMemories(): void {
  ...
  // Context-aware decay: memories accessed recently decay slower
  // accessed_at within 7 days → 0.995 (almost no decay)
  // accessed_at within 30 days → importance-based decay
  // accessed_at older → faster decay
  getDb().prepare(`
    UPDATE memories SET salience = salience * CASE
      ...
      WHEN importance >= 0.5 THEN 0.97
      ELSE 0.93
    END
    WHERE created_at < ? AND pinned = 0
  `).run(now - 7 * 86400, now - 30 * 86400, oneDayAgo);

  // Soft-delete truly dead memories (recoverable for 30 days)
  const softDeleteNow = Math.floor(Date.now() / 1000);
  getDb().prepare(
    'UPDATE memories SET deleted_at = ? WHERE salience < 0.01 AND pinned = 0 AND deleted_at IS NULL'
  ).run(softDeleteNow);

  // Archive before hard-deleting memories soft-deleted more than 30 days ago
  const thirtyDaysAgo = softDeleteNow - 30 * 86400;
  const toArchive = getDb().prepare(
    'SELECT * FROM memories WHERE deleted_at IS NOT NULL AND deleted_at < ?'
  ).all(thirtyDaysAgo) as Memory[];
  if (toArchive.length > 0) {
    const archiveStmt = getDb().prepare(
      `INSERT INTO memory_archive (original_id, source_table, chat_id, content, metadata, archived_at, reason)
       VALUES (?, 'memories', ?, ?, ?, ?, 'decay')`
    );
    const archiveTxn = getDb().transaction(() => {
      for (const mem of toArchive) {
        archiveStmt.run(
          mem.id, mem.chat_id, mem.raw_text,
          JSON.stringify({ summary: mem.summary, topics: mem.topics, entities: mem.entities, importance: mem.importance, source: mem.source }),
          softDeleteNow
        );
      }
    });
    archiveTxn();
    logger.info({ count: toArchive.length }, 'Memories archived before hard-delete');
  }
  getDb().prepare('DELETE FROM memories WHERE deleted_at IS NOT NULL AND deleted_at < ?').run(thirtyDaysAgo);
}

export function undeleteMemory(memoryId: number): boolean {
  const result = getDb().prepare(
    'UPDATE memories SET deleted_at = NULL, salience = 0.5 WHERE id = ? AND deleted_at IS NOT NULL'
  ).run(memoryId);
  return result.changes > 0;
}

export function getDeletedMemories(chatId: string, limit = 20): Memory[] {
  return getDb().prepare(
    'SELECT * FROM memories WHERE chat_id = ? AND deleted_at IS NOT NULL ORDER BY deleted_at DESC LIMIT ?'
  ).all(chatId, limit) as Memory[];
}

export function pinMemory(memoryId: number): void {
  getDb().prepare('UPDATE memories SET pinned = 1 WHERE id = ?').run(memoryId);
}

export function unpinMemory(memoryId: number): void {
  getDb().prepare('UPDATE memories SET pinned = 0 WHERE id = ?').run(memoryId);
}

export function deleteMemory(memoryId: number): void {
  // Soft-delete: set deleted_at instead of hard DELETE
  const now = Math.floor(Date.now() / 1000);
  getDb().prepare('UPDATE memories SET deleted_at = ? WHERE id = ? AND deleted_at IS NULL').run(now, memoryId);
}
...
export function saveConsolidationEmbedding(consolidationId: number, embedding: number[]): void {
  getDb().prepare('UPDATE consolidations SET embedding = ?, embedding_model = ? WHERE id = ?')
...
export function updateMemoryConnections(memoryId: number, connections: Array<{ linked_to: number; relationship: string }>): void {
  const row = getDb().prepare('SELECT connections FROM memories WHERE id = ?').get(memoryId) as { connections: string } | undefined;
  if (!row) return;
  ...
  getDb().prepare('UPDATE memories SET connections = ? WHERE id = ?').run(JSON.stringify(deduped), memoryId);
}

export function markMemoriesConsolidated(ids: number[]): void {
  if (ids.length === 0) return;
  const placeholders = ids.map(() => '?').join(',');
  getDb().prepare(`UPDATE memories SET consolidated = 1 WHERE id IN (${placeholders})`).run(...ids);
}
  ...
  if (keywords.length > 0) {
    ...
    try {
      const ftsResults = getDb()
        .prepare(
          `SELECT consolidations.* FROM consolidations
           JOIN consolidations_fts ON consolidations.id = consolidations_fts.rowid
           WHERE consolidations_fts MATCH ? AND consolidations.chat_id = ?
           ORDER BY rank
           LIMIT ?`,
        )
        .all(ftsQuery, chatId, limit) as Consolidation[];
      if (ftsResults.length > 0) return ftsResults;
    } catch { /* FTS5 table may not exist on older DBs, fall through */ }
  }
...
export function markWaMessageSent(id: number): void {
  const now = Math.floor(Date.now() / 1000);
  getDb().prepare(`UPDATE wa_outbox SET sent_at = ? WHERE id = ?`).run(now, id);
}
 ...
 */
export function pruneWaMessages(retentionDays = 30): { messages: number; outbox: number; map: number } {
  const cutoff = Math.floor(Date.now() / 1000) - retentionDays * 86400;

  const msgResult = getDb().prepare(
    'DELETE FROM wa_messages WHERE created_at < ?',
  ).run(cutoff);

  const outboxResult = getDb().prepare(
    'DELETE FROM wa_outbox WHERE sent_at IS NOT NULL AND created_at < ?',
  ).run(cutoff);

  const mapResult = getDb().prepare(
 ...
 */
export function pruneSlackMessages(retentionDays = 30): number {
  const cutoff = Math.floor(Date.now() / 1000) - retentionDays * 86400;
  const result = getDb().prepare(
...
export function pruneConversationLog(keepPerChat = 500): void {
  ...
  const selectOldStmt = getDb().prepare(`
    SELECT * FROM conversation_log
  ...
  const archiveStmt = getDb().prepare(
    `INSERT INTO memory_archive (original_id, source_table, chat_id, content, metadata, archived_at, reason)
     VALUES (?, 'conversation_log', ?, ?, ?, ?, 'prune')`
  );

  const now = Math.floor(Date.now() / 1000);
  const txn = getDb().transaction(() => {
    for (const chat of chats) {
      const old = selectOldStmt.all(chat.chat_id, chat.chat_id, keepPerChat) as Array<{
        id: number; chat_id: string; role: string; content: string; session_id: string | null; created_at: number;
      }>;
      for (const row of old) {
        archiveStmt.run(
          row.id, row.chat_id, row.content,
          JSON.stringify({ role: row.role, session_id: row.session_id, created_at: row.created_at }),
          now
        );
      }
...
export function getDashboardMemoriesList(chatId: string, limit = 50, offset = 0, sortBy: 'importance' | 'salience' | 'recent' = 'importance', filters?: MemoryListFilters): { memories: Memory[]; total: number } {
  const conditions = ['chat_id = ?', 'deleted_at IS NULL'];
  ...
  const memories = getDb()
    .prepare(
      `SELECT * FROM memories WHERE ${whereClause} ${orderClause} LIMIT ? OFFSET ?`,
    )
    .all(...params, limit, offset) as Memory[];
  return { memories, total: total.cnt };
}

export function getMemoryTopics(chatId: string): { topic: string; count: number }[] {
  return getDb()
    .prepare(
      `SELECT json_each.value as topic, COUNT(*) as count
       FROM memories, json_each(memories.topics)
       WHERE memories.chat_id = ? AND memories.deleted_at IS NULL
       GROUP BY json_each.value
       ORDER BY count DESC`,
    )
    .all(chatId) as { topic: string; count: number }[];
}
...
export function createMissionTask(
  ...
  createdBy = 'dashboard',
  priority = 0,
  nextMissionId: string | null = null,
): void {
  const now = Math.floor(Date.now() / 1000);
  getDb().prepare(
    `INSERT INTO mission_tasks (id, title, prompt, assigned_agent, status, created_by, priority, created_at, next_mission_id)
     VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?)`,
  ).run(id, title, prompt, assignedAgent, createdBy, priority, now, nextMissionId);
}
...
export function getMissionTask(id: string): MissionTask | null {
  return (getDb().prepare('SELECT * FROM mission_tasks WHERE id = ?').get(id) as MissionTask) ?? null;
}
...
export function completeMissionTask(
  ...
  // Mission chaining: if completed and there's a next_mission_id, log activation
  if (status === 'completed') {
    const task = getDb().prepare('SELECT next_mission_id FROM mission_tasks WHERE id = ?').get(id) as { next_mission_id: string | null } | undefined;
    if (task?.next_mission_id) {
      const next = getDb().prepare('SELECT status FROM mission_tasks WHERE id = ?').get(task.next_mission_id) as { status: string } | undefined;
      if (next && next.status === 'queued') {
        logger.info({ from: id, to: task.next_mission_id }, 'Mission chain: activating next task');
      }
    }
  }
}
...
export function cleanupOldMissionTasks(olderThanDays = 30): number {
...
export function getAuditLogCount(agentId?: string): number {
  if (agentId) {
    return (getDb().prepare('SELECT COUNT(*) as c FROM audit_log WHERE agent_id = ?').get(agentId) as { c: number }).c;
  }
