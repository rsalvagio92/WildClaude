...
## External Service Dashboards (legacy)

> **Superseded by [Declarative Dashboards](#declarative-dashboards).** The `http` widget source (with `{{SECRET}}` substitution) covers the same APIs; the built-in `connected-services` template wires GitHub + Vercel out of the box. This module no longer has a dashboard nav entry — its API stays for backward compatibility with user-defined services in `config.json`.
...
**URL:** `http://<host>:3141` (or `https://<host>:3141` when `DASHBOARD_HTTPS=true` — see [HTTPS](#https-self-signed))
**Stack:** Hono backend + a framework-free ES-module SPA served from `dashboard-ui/` (no build step, no CDN). Static assets under `/ui/*`. (The legacy single-file dashboard and `/legacy` route were removed — the SPA is the only UI.)
...
20 modules, grouped: **Chat** (Command Center) · **Projects** (Projects) · **Knowledge** (Memory Palace, Knowledge Wiki, Daily Journal, Reflection & Digest) · **Agents** (Agent Hub, Mission Control, Automation, Workflows, Evals) · **Ecosystem** (Dashboards, Skills & MCP) · **Monitoring** (System Vitals, Trace Inspector, Live Activity, Audit Log, Hermes Lab) · **System** (File Explorer, Settings). The former standalone *Marketplace* is now the **Browse** tab inside *Skills & MCP*; *External Dashboards* was folded into the declarative **Dashboards** builder.
...
Set `DASHBOARD_HTTPS=true` to serve the dashboard over HTTPS. A self-signed cert is generated by [src/dashboard-tls.ts](src/dashboard-tls.ts) (via `selfsigned`, pure-JS) and cached in `USER_DATA_DIR` with SANs for `localhost`, `127.0.0.1`, and every LAN/Tailscale IPv4. **Required for in-browser microphone / Web Speech voice input over a LAN** — browsers block the mic on plain `http://<lan-ip>` (non-secure context). Accept the cert once per device. Telegram dashboard links follow the scheme automatically.

## Declarative Dashboards

The **Dashboards** module ([src/dashboards-v2.ts](src/dashboards-v2.ts), templates in [src/dashboard-templates.ts](src/dashboard-templates.ts)) builds dashboards from a **JSON widget spec** resolved server-side (secrets/CORS never reach the browser). Specs live in `USER_DATA_DIR/dashboards/<id>.json`. API base: `/api/dash` (distinct from the legacy `/api/dashboards`).

- **Widget types:** `metric`, `chart`, `table`, `list`, `feed`, `form`, `note`, `gauge` (progress to a target), `insight` (on-demand AI read of logged data).
- **Sources:** `http` (JSON API + dotted `jsonPath` + `{{SECRET}}` substitution, SSRF-guarded), `rss`, `local` (tracker data in the `dashboard_data` table — `form` widgets write, readers read via `config.readWidget`), `static`. Local aggregations: `sum`/`avg`/`count`/`last`/`list`/`streak`/`delta`.
- **Create:** from a template, a plain-language prompt (LLM-generated), or a raw spec. Guided 2-step flow: `POST /api/dash/plan` returns recommended widgets + clarifying questions; the answers feed generation.
- **Voice autofill:** form widgets transcribe via the browser Web Speech API; `POST /api/dash/:id/parse-entry` maps free text → fields (Haiku). Requires HTTPS (secure context).
- **Refine:** `POST /api/dash/:id/refine` updates an existing spec from a plain-language/voice instruction, **preserving widget ids** so logged data survives. Plus edit mode (resize/reorder/delete) + raw-spec editor.
- **AI insight:** `POST /api/dash/:id/insight/:wid`. Default templates: `markets-crypto`, `fitness-nutrition`, `news-briefing`, `connected-services` (GitHub + Vercel via `http`+secret).
- Telegram: `/dashboard_create <description>`, `/dashboard_edit <id> <instruction>` (instruction may be a transcribed voice message).

## Project Containers

A **project** ([src/projects.ts](src/projects.ts)) bundles everything WildClaude needs to work on a repo/app: description, repos, environment notes, **secret references (names only — never values)**, links, a markdown **knowledge base**, and project-scoped dashboards. Stored in `USER_DATA_DIR/projects/<id>/` (`project.json` + `knowledge/`).

- `buildProjectReference()` injects the active project's context (repos/env/KB/secret availability) into the system prompt. Active project is **per-chat**; set with `/project use <id>` (or in the dashboard).
- **Auto-detect → propose** ([src/project-detect.ts](src/project-detect.ts)): when a message mentions a new project/repo, a cheap pre-filter gates a Haiku extraction and the bot **proposes** creating a container via a Telegram inline keyboard (accept creates a stub; details accrue into its KB/Wiki). Never silent; guarded by confidence floor, dedupe, and cooldown.
- API: `/api/projects` (+ `/:id`, `/:id/knowledge[/:file]`, `/:id/reference`, `/api/projects/active/:chatId`). Telegram: `/project [use <id> | none]`.

## Knowledge Wiki

[src/wiki.ts](src/wiki.ts) — durable, editable, **topic-keyed** articles layered on `memory_blocks` (owner `wiki` = published, `wiki-draft` = suggested). Articles don't decay.

- **Recall on mention:** on every chat message, any published article whose topic appears in the text is injected as reference context (cheap substring match, capped — safe on the hot path).
- **Curation:** `distillArticle(topic)` drafts/expands an article from matching memories + notes (Haiku). `runWikiCuration()` finds recurring high-importance memory topics lacking an article and drafts them (auto-cron `__internal:wiki_curate:run` at 05:00; also manual). Drafts await **Approve/Discard**.
- API: `/api/wiki` (+ `/:id`, `/:id/approve`, `/distill`, `/curate`). UI: **Knowledge → Wiki**.

## Workflow & Eval Authoring

Both [src/workflows.ts](src/workflows.ts) and [src/evals.ts](src/evals.ts) support full authoring: object-level validators + `getRaw`/`validate`/`save`/`delete` helpers and `generate*()` (LLM "describe it"). APIs: `POST /api/{workflows,evals}` (save), `/validate`, `/generate`, `GET .../raw/:name`, `DELETE .../:name`. UI: shared `dashboard-ui/js/yaml-authoring.js` (editor with live validation; generator hands off to the editor for review). Both modules gain **✨ Describe · + New · Edit · Delete**.
...
- **Recommended skills** ([src/recommended-skills.ts](src/recommended-skills.ts)) — curated picks: book-to-skill, graphify, Skill_Seekers, claude-skills-313. Surfaced in the dashboard's **Skills & MCP → Browse** tab.
...
| `/dashboard_create <description>` | Generate a custom dashboard from a description |
| `/dashboard_edit <id> <instruction>` | Improve a dashboard by prompt or (transcribed) voice |
| `/project [use <id> \| none]` | Show or set the active project container for this chat |
| **Hermes** | |
...
│   ├── external-dashboards.ts # Legacy service connectors (superseded by dashboards-v2)
│   ├── dashboards-v2.ts      # Declarative dashboard engine (specs, sources, LLM gen/refine)
│   ├── dashboard-templates.ts # Built-in dashboard templates
│   ├── projects.ts           # Project containers (KB, secret refs, scoped dashboards)
│   ├── project-detect.ts     # Auto-detect project mentions → propose container
│   ├── wiki.ts               # Knowledge Wiki (topic articles over memory_blocks)
│   ├── skill-sync.ts         # Bridge USER_DATA_DIR/skills → ~/.claude/skills
│   ├── dashboard.ts          # Hono web server (HTTP or self-signed HTTPS)
│   ├── dashboard-tls.ts      # Self-signed cert for DASHBOARD_HTTPS
│   ├── config.ts             # Environment config
│   ├── whatsapp.ts           # WhatsApp bridge
│   └── slack.ts              # Slack bridge
│   # UI: SPA lives in dashboard-ui/ (the legacy single-file dashboard-html.ts was removed)
